...
- **BREAKING** Rename the canonical block and its nested Universal Editor models to the `lixiang-product-detail-picture-group` namespace.
- Migrate all existing `Picture Group` content under `/content/demo-site`, including container and nested item model metadata.
...
- Content migration: three existing container nodes and their nested group/media item metadata under the Li-L6 active, aligned, and backup pages.
