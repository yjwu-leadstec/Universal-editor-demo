...
- Treat lazy offscreen images as pending rather than broken. Recovery is driven by the browser error event or by a completed zero-width image after it reaches the viewport; the shared media helper must not remove deep-page Author images after an arbitrary one-second timeout.
...
- Author images are far below the initial viewport on Li-L6. A timeout-based broken-image heuristic produced false fallbacks there; explicit load errors and completed failures observed near the viewport remain recoverable without racing lazy loading.
