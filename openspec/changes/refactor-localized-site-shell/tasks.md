...
- [ ] 1.1 导出并记录根级和 language-master 的 nav/footer JCR 路径、发布状态、`.plain.html` 与四断点截图。
- [ ] 1.2 生成 header/footer/locale links 清单，标记根相对链接、AEM 引用、外部链接、404 和跨语言错误。
...
- [ ] 2.1 新增无 DOM 依赖的 locale-root/path resolver 纯函数模块。
- [ ] 2.2 实现 metadata override → localized sibling → migration root fallback 优先级。
- [ ] 2.3 限制 override 为安全同源站内路径，拒绝危险 scheme 和歧义路径。
...
- [ ] 3.1 新增单例 `header-settings` model，配置 localeDirectory 引用和全部本地化交互/accessible labels。
- [ ] 3.2 新增 `locale-directory` container 与 `locale-option` item definitions/models/filter。
- [ ] 3.3 配置 marketCode、marketLabel、languageTag、languageLabel、link、enabled、textDirection 字段与校验。
- [ ] 3.4 确保模型为单层 container + flat items，不使用 nested multifield。
- [ ] 3.5 生成语义化 anchors/list DOM；无效项跳过，不生成 `#`。
- [ ] 3.6 增加重复 key/link、同组 label 不一致、无 enabled items 的防御性校验。
- [ ] 3.7 运行 `npm run build:json` 并检查生成文件只包含预期模型变化。

## 4. Header refactor

- [ ] 4.1 使用 shared resolver 替换固定 `/nav` fallback，同时保留 metadata override 和迁移开关。
- [ ] 4.2 移除 `LOCALE_MARKETS`、固定 dialog title 和所有业务目的地硬编码。
- [ ] 4.3 从 `header-settings` 显式引用/当前 locale root 加载 locale directory，并复用同一数据生成桌面和移动视图。
- [ ] 4.4 先校验 fragment 最小契约再增强 DOM；失败时保留真实语言入口链接。
- [ ] 4.5 从 `header-settings` 读取主导航、菜单、返回、语言与关闭 accessible labels；settings 无效时保留 semantic source nav。
- [ ] 4.6 保持品牌、主导航、mega panel 与 cards 的现有语义兼容和 instrumentation。
- [ ] 4.7 修复 link/current-state 规范化，不用 `#` 代替缺失 href。
- [ ] 4.8 验证 desktop panel、dialog、mobile drawer 的 keyboard/focus/ARIA/reduced-motion。

## 5. Footer refactor

- [ ] 5.1 使用 shared resolver 替换固定 `/footer` fallback。
- [ ] 5.2 保留 heading + list 列模型和现有简单 footer 兼容。
- [ ] 5.3 将回到顶部 label/link 改为作者内容，JS 只做增强；缺失时不渲染。
- [ ] 5.4 保持 desktop/mobile 双视图 instrumentation 和 accordion ARIA。
- [ ] 5.5 处理缺失 fragment、空列、长标签、RTL、legal/social 可选内容。

## 6. Document locale and links

- [ ] 6.1 用 metadata/locale root 设置 `<html lang>`，移除固定 `en`。
- [ ] 6.2 在 eager 阶段按 metadata/BCP 47 技术规则设置 `<html dir>`，再与当前 locale option 的 textDirection 校验，覆盖 LTR 与 RTL。
- [ ] 6.3 为 locale anchors 输出 `hreflang`，为当前项输出 `aria-current`。
- [ ] 6.4 保持完整 SEO alternate/canonical 映射在本变更之外，并记录后续需求。

## 7. AEM authoring and MSM setup

- [ ] 7.1 在 `/language-master/en` 创建/完善 nav、footer、locale-directory 源内容；不修改旧 `/site/nav`。
- [ ] 7.2 把母版 nav/footer 的根相对内部链接替换为语言根 AEM 内容引用。
...
- [ ] 8.5 运行 `npm run lint`、`npm run build:json`、resolver/parser tests 和浏览器 smoke tests。
