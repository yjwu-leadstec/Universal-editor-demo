...
- 实际测试内容：`/content/demo-site/language-master/en/li-l6/jcr:content/root/section/block-{03,05,12,15,20,24}-lixiang-product-full-screen-intro`。
...
- 组件、模型、CSS/JS namespace 只使用 `lixiang-product-full-screen-intro`；不保留 `chapter-intro` loader、selector 或标题兼容分支。
...
- Universal Editor Author 使用 16:9 有界画布；专用规则必须位于全部响应式媒体查询之后，且选择器权重必须高于对应断点规则。

## 回归标准

- 背景图/视频保持 16:9 桌面构图和移动端全屏构图。
- Preview 在 1920px、1440px、1024px、768px、390px 不产生页面级横向滚动或 `.block-error`。
- Author 中 6 个实例均为 `data-block-status="loaded"`，保持 16:9 有界高度，旧 `chapter-intro` 实例数为 0。
