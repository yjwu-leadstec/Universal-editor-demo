...
| 第 2 段 | **Primary 主导航** | `Header Navigation` 容器；按顺序添加 Top Navigation、Panel Group Heading、Panel Card 子项 |
...
| 子项 | 字段 | 用途 |
| --- | --- | --- |
| Top Navigation | Navigation Label、Destination | 新建一级菜单；Destination 使用 AEM 内容选择器，从 `/content/demo-site` 选页面 |
| Panel Group Heading | Group Heading | 给最近的一级菜单新增面板分组 |
| Panel Card | Card Title、Destination、Card Images、Card Description | 给最近的一级菜单/分组新增卡片；图片第一张为背景，第二张为可选透明字标 |

容器自身的 `Default Card Action Label` 配置当前语言共享 CTA，例如 `Learn More`。内容顺序就是归属关系：一个 Top Navigation 后面的 Group/Card 都属于它，直到出现下一个 Top Navigation。

- 卡片 = 车型名（title）+ 定位文案（description）+ 车图（media）+ 容器共享 CTA。
- 分组示例（截图）：**Battery Electric Vehicle**（Li i6 / Li i8 / Li MEGA）、**REEV Vehicle**（Li L6 / L7 / L8 / L9）。分组名来自 Panel Group Heading。
...
3. 第 2 段选择 `Header Navigation`；通过 Add 添加 Top Navigation / Panel Group Heading / Panel Card。
4. 编辑 Top 或 Card 时，点击 Destination 的内容选择按钮，从 `/content/demo-site` 选择页面；不要手输 `/service` 之类根相对路径。
5. 通过 Content Tree 拖动条目调整顺序；Group/Card 必须放在所属 Top Navigation 后面。
