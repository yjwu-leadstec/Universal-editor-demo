...
> 测试资产说明：Header Navigation 当前仅引用已复制到 `/content/dam/li-auto/shared/vehicles/<model>/homepage` 的正式路径；`/content/dam/li-auto/intake/global/header-navigation-test` 中的原始测试文件只作来源留痕与验证，不被页面引用。生产上线前仍须由客户确认官网来源素材的使用权并完成资产审批；不得从 `intake` 向生产页面提供素材。
