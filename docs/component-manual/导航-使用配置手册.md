...
| Dropdown Section Heading | Section Heading | 给当前所属的一级菜单新增面板分组 |
| Dropdown Link Card | Link Card Title、Target Page or URL、Card Background Image、Vehicle Foreground Image、Short Description | 给当前所属的一级菜单/分组新增卡片；背景图和可选透明底整车前景分别从 DAM 选择，不再依赖图片选择顺序 |
...
- 卡片 = 车型名（title）+ 定位文案（description）+ 背景图（backgroundImage）+ 可选透明底整车前景（foregroundImage）+ 容器共享 CTA。
...
4. 在画布中选中一张编辑卡片并打开 Properties。编辑 Link 或 Card 时，点击 `Target Page or URL` 右侧的文件夹按钮，从 `/content/demo-site` 选择页面；不要手输 `/service` 之类根相对路径。车型卡片分别通过 `Card Background Image` 和 `Vehicle Foreground Image` 选择 DAM 资源；前景应使用透明 PNG 整车切图，不要使用已经包含车型文字的复合 Tile，也不要再把两张图片放进一个多选字段。
...
> 迁移说明：代码仍兼容旧 Core Text 列表，但当前 `language-master/en/nav` 已完成结构化迁移并删除冗余的 `section_0/text`。后续不要同时维护两套 Author 数据；只保留 `Header Navigation` 作为主导航事实来源。

> 测试资产说明：Header Navigation 当前仅引用已复制到 `/content/dam/li-auto/shared/vehicles/<model>/homepage` 的正式路径；`/content/dam/li-auto/intake/global/header-navigation-test` 中的原始测试文件只作来源留痕与验证，不被页面引用。临时验证资源统一放入 `/content/dam/li-auto/test/codex/nav-validation`，不得被正式页面引用，验证结束后应清空。生产上线前仍须由客户确认官网来源素材的使用权并完成资产审批；不得从 `intake` 或 `test` 向生产页面提供素材。

当前 `language-master/en` 已按以下规则完成结构化配置：

- Li i6、i8、MEGA、L6、L7、L8、L9 均指向同一语言根下的站内页面，图片分别取自 `/content/dam/li-auto/shared/vehicles/<model>/homepage`。
- 每个车型目录使用 `<model>-navigation-card-background.jpg` 与 `<model>-navigation-card-foreground.png`；不得把官网复合图同时填入两个字段。
- About Us、Join Us、Investor Relations 指向同一语言根下的站内页面，背景图取自 `/content/dam/li-auto/shared/corporate/navigation`；企业卡片不要求车型前景图。
- 新建的空壳目标页只用于保证路由存在，不代表车型/企业页面正文已经完成迁移。发布 Nav 前先发布这些目标页和被引用 DAM 资源。
- 旧 Core Text 导航只用于迁移回退。确认结构化 Nav 在 Author、Preview、Live 均通过后，先创建 git 检查点和 AEM 版本，再只删除 `section_0/text`；Logo、语言入口、Header Settings 不属于冗余项。
