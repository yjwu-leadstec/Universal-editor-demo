...
**优先级 P1** · 对应 block：`lixiang-product-intro-slider` → 子项 `lixiang-product-intro-slider-slide`
...
**容器 `lixiang-product-intro-slider`**

| 字段 | 类型 | Label | 选项 / 说明 |
| --- | --- | --- | --- |
| title | textarea | Desktop Title | 区块标题（PC），支持手动换行 |
| mobileTitle | textarea | Mobile Title Override | 移动端标题覆盖，支持独立换行（可空则跟随 PC） |
| description | richtext | Description | 区块描述 |
| autoPlay | boolean | Desktop Autoplay | 桌面自动轮播开关 |
| interval | number | Autoplay Interval (seconds) | 自动切换间隔秒数（默认 **4s**） |
| showVideoControl | boolean | Show Video Play / Pause | 视频右上角播放/暂停按钮 |
| showProgress | boolean | Show Video Progress | 播放/暂停按钮上的视频进度环 |
| headingColor | select | Section Heading Color | 白 `#FFFFFF` / 黑 `#191919` |
| accentColor | text | Indicator Color | 可选六位十六进制色值，默认黑 `#191919` |
| classes | multiselect | Style | 背景白 `#FFFFFF` / 黑 `#000000` / 灰 `#FAFAFA` + 间距 PC `160/80/0px`、M `80/60/0px`（最多选 2） |
| id | text | ID | 可空 |

**子项 `lixiang-product-intro-slider-slide`（每屏 Banner，可增删排序）**
...
| title | textarea | Card Title | 主标题（如车型名），支持手动换行 |
| description | richtext | Card Description | 副文案 |
| note | textarea | Note | 小字描述，支持手动换行 |
| showNote | boolean | Show Note | 勾选后显示备注；旧内容默认兼容显示已有备注 |
| copyColor | select | Desktop Card Text Color | 根据素材选择白 `#FFFFFF` / 黑 `#191919` |
| indicatorLabel | textarea | Indicator Label | 可选指示条文案，支持两行及手动换行 |
...
| imageAlt | text | Image Alt Text | — |
| video | reference | Video (optional) | AEM Assets MP4 引用。**仅居中卡片自动播放**，结束显示 Replay |
...
2. 填 eyebrow + 大标题；逐张加 `Carousel Card`（poster 必填、视频从 AEM Assets 选择、action 文案可选）。
3. ≥3 张才出指示器，注意卡片数量规则。
...
| 轮播 Banner | `lixiang-product-intro-slider → lixiang-product-intro-slider-slide` | 主题×间距；卡 `linkType` | 2.35:1、5s 自动、指示器热区 |
