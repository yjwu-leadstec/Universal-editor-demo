...
function createGuideCard(item) {
  ...
  card.className = 'lixiang-product-guide-card';
  ...
  copy.className = 'lixiang-product-guide-copy';
  ...
  if (descriptionSource?.textContent.trim()) {
    const description = createRichText(descriptionSource, 'lixiang-product-guide-description');
...
export default function decorate(block) {
  ...
  shell.className = 'lixiang-product-guide-shell';
  ...
  list.className = 'lixiang-product-guide-list';
  modelItems(block, 'lixiang-product-guide-item').forEach((item) => list.append(createGuideCard(item)));
  if (list.childElementCount) shell.append(list);
  addBlockAnchor(block, block, shell);
  block.replaceChildren(shell);
  revealElements(block, '.lixiang-product-guide-card');
}
