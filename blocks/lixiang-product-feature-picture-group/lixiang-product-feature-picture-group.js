...
function createCard(item, headingTag = 'h3') {
  ...
  card.className = 'lixiang-product-feature-picture-group-card';
  const { element: media } = createMedia(item, { fallbackLabel: '图片' });
  card.append(media);

  const copy = document.createElement('div');
  copy.className = 'lixiang-product-feature-picture-group-copy';
  ...
  if (descriptionSource?.textContent.trim()) {
    const description = createRichText(descriptionSource, 'lixiang-product-feature-picture-group-description');
...
function createGroup(group, items) {
  ...
  section.className = 'lixiang-product-feature-picture-group-group';
  const title = group ? propText(group, 'title') : '';
  if (title) {
    const heading = document.createElement('h3');
    heading.className = 'lixiang-product-feature-picture-group-group-title';
    heading.textContent = title;
    instrumentProp(group, 'title', heading);
    section.append(heading);
  }
  const grid = document.createElement('div');
  grid.className = 'lixiang-product-feature-picture-group-list';
...
export default function decorate(block) {
  ...
  shell.className = 'lixiang-product-feature-picture-group-shell';
  const header = createSectionHeader(block);
  if (header.childElementCount) shell.append(header);

  const groups = modelItems(block, 'lixiang-product-feature-picture-group-group');
  if (groups.length) {
    groups.forEach((group) => {
      const items = modelItems(group, 'lixiang-product-feature-picture-group-card');
    ...
    const looseItems = [...block.children]
      .filter((el) => el.matches('[data-aue-model="lixiang-product-feature-picture-group-card"]'));
    if (looseItems.length) shell.append(createGroup(null, looseItems));
  } else {
    const items = modelItems(block, 'lixiang-product-feature-picture-group-card');
  ...
  if (note) {
    ...
    element.className = 'lixiang-product-feature-picture-group-note';
  ...
  revealElements(
    block,
    '.product-section-header, .lixiang-product-feature-picture-group-group-title, .lixiang-product-feature-picture-group-card, .lixiang-product-feature-picture-group-note',
  );
}
