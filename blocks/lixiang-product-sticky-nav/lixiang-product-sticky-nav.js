...
export default function decorate(block) {
  ...
  carName.href = `#${document.querySelector('main .lixiang-product-hero[id]')?.id || 'top'}`;
  carName.textContent = vehicleName;
  instrumentProp(block, 'carName', carName);
  const list = document.createElement('div');
  list.className = 'product-sticky-links';
  modelItems(block, 'lixiang-product-sticky-nav-item').forEach((item) => {
  ...
  window.requestAnimationFrame(() => observeTargets([...list.querySelectorAll('a')]));
}
