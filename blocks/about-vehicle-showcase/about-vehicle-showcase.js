...
export default function decorate(block) {
  ...
  const models = modelItems(block, 'about-vehicle-model');
  if (models.length) {
    const list = document.createElement('div');
    list.className = 'about-vehicle-models';
    models.forEach((item) => {
      const name = propText(item, 'name');
      const description = propText(item, 'description');
      if (!name && !description) return;
      const card = document.createElement('div');
      card.className = 'about-vehicle-model';
      if (name) {
        const nameEl = document.createElement('p');
        nameEl.className = 'about-vehicle-model-name';
      ...
      if (description) {
        ...
        descEl.className = 'about-vehicle-model-desc';
