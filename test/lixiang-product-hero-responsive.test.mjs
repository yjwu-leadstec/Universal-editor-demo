...
const [heroCss, heroJs, heroModel, productUtils] = await Promise.all([
  readFile(new URL('../blocks/lixiang-product-hero/lixiang-product-hero.css', import.meta.url), 'utf8'),
  readFile(new URL('../blocks/lixiang-product-hero/lixiang-product-hero.js', import.meta.url), 'utf8'),
  readFile(new URL('../blocks/lixiang-product-hero/_lixiang-product-hero.json', import.meta.url), 'utf8'),
...
test('hero scroll cue skips the removed L6 sticky navigation', () => {
  assert.match(heroJs, /classList\.contains\('lixiang-product-sticky-nav-wrapper'\)/);
...
test('product hero authors four responsive poster compositions', () => {
  ...
  const hero = models.find(({ id }) => id === 'lixiang-product-hero');
  const responsive = models.find(({ id }) => id === 'lixiang-product-hero-responsive-media');
...
test('Universal Editor caps product hero height after responsive viewport rules', () => {
  const authorRule = '.adobe-ue-edit main .lixiang-product-hero';
  assert.ok(
    heroCss.lastIndexOf(authorRule) > heroCss.lastIndexOf('@media (width <= 720px)'),
  );
  assert.match(
    heroCss,
    /\.adobe-ue-edit main \.lixiang-product-hero\s*\{[^}]*min-height:\s*clamp\(720px, 100vh, 1080px\)/s,
  );
});
