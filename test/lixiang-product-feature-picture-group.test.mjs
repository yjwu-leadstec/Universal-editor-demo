...
const base = '../blocks/lixiang-product-feature-picture-group/lixiang-product-feature-picture-group';
const [css, js, modelRaw] = await Promise.all([
  ...
  readFile(new URL(`${base.replace(/\/lixiang-product-feature-picture-group$/, '/_lixiang-product-feature-picture-group')}.json`, import.meta.url), 'utf8'),
]);
...
test('decorate reuses the shared product-block helpers and stays UE-editable', () => {
  ...
  assert.match(js, /modelItems\(block, 'lixiang-product-feature-picture-group-group'\)/);
  assert.match(js, /modelItems\(group, 'lixiang-product-feature-picture-group-card'\)/);
  assert.match(js, /modelItems\(block, 'lixiang-product-feature-picture-group-card'\)/);
...
test('cards are image-fill tiles with overlaid copy (design: radius 10, 22/16px)', () => {
  assert.match(css, /\.lixiang-product-feature-picture-group-list\s*\{[^}]*grid-template-columns:\s*repeat\(3, minmax\(0, 1fr\)\)/);
  assert.match(css, /\.lixiang-product-feature-picture-group-card\s*\{[^}]*border-radius:\s*10px/);
  assert.match(css, /\.lixiang-product-feature-picture-group-card \.product-media\s*\{[^}]*position:\s*absolute;[^}]*inset:\s*0/);
  assert.match(css, /\.lixiang-product-feature-picture-group-copy\s*\{[\s\S]*position:\s*absolute[\s\S]*color:\s*#fff/);
  assert.match(css, /\.lixiang-product-feature-picture-group-copy h3\s*\{[\s\S]*font-size:\s*22px/);
  assert.match(css, /\.lixiang-product-feature-picture-group-description\s*\{[\s\S]*font-size:\s*16px/);
});
...
test('placeholder tile overrides the shared grey to the L6 grey (light #b6b6b6 / dark #5d5d5d)', () => {
  ...
  assert.match(css, /\.lixiang-product-feature-picture-group\.dark[\s\S]*is-media-fallback::before[\s\S]*background:\s*#5d5d5d/);
});

test('mobile collapses to a horizontal snap gallery', () => {
  assert.match(css, /@media \(width <= 719px\)[\s\S]*\.lixiang-product-feature-picture-group-list\s*\{[\s\S]*grid-auto-flow:\s*column[\s\S]*scroll-snap-type:\s*x mandatory/);
});

test('UE model exposes background variants and drops the link fields', () => {
  const ids = model.models.map((m) => m.id);
  assert.deepEqual(ids, [
    'lixiang-product-feature-picture-group',
    'lixiang-product-feature-picture-group-group',
    'lixiang-product-feature-picture-group-card',
  ]);
  const block = model.models.find((m) => m.id === 'lixiang-product-feature-picture-group');
  ...
  const card = model.models.find((m) => m.id === 'lixiang-product-feature-picture-group-card');
  ...
  assert.deepEqual(
    model.filters.find((f) => f.id === 'lixiang-product-feature-picture-group').components,
    ['lixiang-product-feature-picture-group-group', 'lixiang-product-feature-picture-group-card'],
  );
  assert.deepEqual(
    model.filters.find((f) => f.id === 'lixiang-product-feature-picture-group-group').components,
    ['lixiang-product-feature-picture-group-card'],
  );
});
