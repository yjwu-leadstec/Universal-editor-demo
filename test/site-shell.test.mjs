...
test('normalizes delivery and fragment paths', () => {
  assert.equal(normalizePath('/global/en/nav.plain.html?x=1'), '/global/en/nav');
...
test('accepts only same-origin fragment overrides', () => {
  assert.equal(safeFragmentOverride('/global/en/nav.html', ORIGIN), '/global/en/nav');
...
test('rebases only same-origin page links into the active locale root', () => {
  ...
  assert.equal(localizeSiteHref('/global/en', '/language-master/en', ORIGIN), '/global/en');
...
test('derives document language and direction', () => {
  assert.deepEqual(getDocumentLocale({ pathname: '/global/en/homepage' }), {
    languageTag: 'en', direction: 'ltr',
  });
  assert.deepEqual(getDocumentLocale({ pathname: '/bh/ar' }), {
    languageTag: 'ar', direction: 'rtl',
  });
  assert.deepEqual(getDocumentLocale({
    pathname: '/bh/ar', language: 'en-GB', direction: 'ltr',
